...
Create both projects first, then fill them in below. **ServiceDefaults is not a plain `classlib`** —
it comes from a dedicated Aspire template that brings the OpenTelemetry / health-check / resilience
wiring with it, so you scaffold it and edit rather than writing it by hand.

```powershell
# One-time: install the Aspire templates (skip if already present)
dotnet new install Aspire.ProjectTemplates

# Shared — a plain class library for cross-project constants
dotnet new classlib -o src/Shared
dotnet sln CrmApp.slnx add src/Shared/Shared.csproj
dotnet add src/Infrastructure reference src/Shared   # Infrastructure looks up the connection string by name

# ServiceDefaults — Aspire template (OTel, health checks, service discovery, HTTP resilience)
dotnet new aspire-servicedefaults -o src/ServiceDefaults
dotnet sln CrmApp.slnx add src/ServiceDefaults/ServiceDefaults.csproj
```

Web picks up ServiceDefaults in Phase 6; AppHost references both Web and Shared in Phase 7.
...
dotnet sln CrmApp.slnx add src/Web/Web.csproj
...
The AppHost is scaffolded from the Aspire orchestrator template (it adds `Aspire.AppHost.Sdk`, not a
plain SDK), then wired to the projects it runs:

```powershell
# AppHost — Aspire orchestrator template
dotnet new aspire-apphost -o src/AppHost
dotnet sln CrmApp.slnx add src/AppHost/AppHost.csproj

# Recent templates scaffold the entry file as AppHost.cs; this repo standardizes on Program.cs.
# Rename-Item's second arg is a bare name, not a path (skip if the template already emitted Program.cs).
Rename-Item src/AppHost/AppHost.cs Program.cs

# Orchestrates Web and needs the Shared resource-name constants.
# The Web reference is what generates the strongly-typed `Projects.Web` used in Program.cs below.
dotnet add src/AppHost reference src/Web src/Shared
```

> ⚠️ Mark the Shared reference so Aspire does **not** treat the constants library as a resource to
> launch — open `src/AppHost/AppHost.csproj` and add the attribute:
>
> ```xml
> <ProjectReference Include="..\Shared\Shared.csproj" IsAspireProjectResource="false" />
> ```
>
> Web is a real orchestrated resource and stays as the plain `dotnet add reference` default.
